<?php /*
  ...
  Plugin URI:
  Description: simple css editor for wp-login page
  Author: Tiago Anastácio
  Author URI: ittca.eu
  Version: 1.1.1
*/
if(!defined('ABSPATH'))exit;
if(!function_exists('twpl_LoginFiles')){
  function twpl_LoginFiles() {wp_enqueue_style('twpl_style', plugins_url('style.css',__FILE__ ));}
  add_action( 'admin_init','twpl_LoginFiles');
}
...
if(!function_exists('twped_csm')){
  function twped_csm($slug){global $submenu;if(isset($submenu['twp_editor'])){foreach($submenu['twp_editor'] as $subm){if($subm[2]==$slug)return 1;}}}
}
if (!function_exists('twptwp_login')){
  function twptwp_login(){
  ...
  add_action('admin_menu', 'twptwp_login');
}
...
if(!function_exists('twpl_install')){
  function twpl_install(){
    ...
    if(!$wpdb->get_results("select * from ".$wpdb->prefix."twpLogin where id = 1")){
      $wpdb->query("create table if not exists ".$wpdb->prefix."twpLogin(id int, logo varchar(150), bg varchar(10), formbg varchar(10), formtext varchar(10),  btncolor varchar(10), bgtextcolor varchar(10), btntxtcolor varchar(10), d int, h int, hpath varchar(50), shadow int)");
      $wpdb->query("insert into ".$wpdb->prefix."twpLogin values(1,'','','','','','','',0,0,'',0)");
    }
  }
    ...
    <style type="text/css"> <?php
      if($data[0]->logo){ ?>
        body.login div#login h1 a{
          background-image: url(<?php echo esc_url($logo); ?>); <?php 
          if($data[0]->shadow){ ?> 
            filter: drop-shadow(4px 4px 4px rgba(0,0,0,0.7)); <?php
          } ?>
        } <?php
      }
      if($data[0]->bg){ ?> body{background-color:<?php echo esc_attr($data[0]->bg)?> !important;} <?php } ?>
      .login #login_error, .login .message, .login .success, #loginform, #lostpasswordform{ <?php
        if($data[0]->formbg){ ?> background: <?php echo esc_attr($data[0]->formbg) ?> !important; <?php }
        if($data[0]->formtext){ ?> color:<?php echo esc_attr($data[0]->formtext) ?> !important; <?php } ?>
      } <?php
      if($data[0]->shadow){ ?>
        .login #login_error, .login .message, .login .success, #loginform, #lostpasswordform, #loginform, .login #login_error, .login .message, .login .success, #lostpasswordform{
          -webkit-box-shadow: 1px 1px 8px 2px rgba(0,0,0,0.55) !important;
          box-shadow: 1px 1px 8px 2px rgba(0,0,0,0.55) !important;
        }
        #language-switcher input, .wp-core-ui .button.button-large, #language-switcher select{
          -webkit-box-shadow: 1px 1px 1px 1px rgba(0,0,0,0.55);
          box-shadow: 1px 1px 1px 1px rgba(0,0,0,0.55);
        }
        body.login div#login h1 a{ filter: drop-shadow(4px 4px 4px rgba(0,0,0,0.7));} <?php
      } ?>
      ...
      .wp-core-ui .button.button-large{
        border-radius:5px;
        background:<?php echo $data[0]->btncolor ?>;
        color:<?php echo $data[0]->btntxtcolor ?>;
      ...
      if($data[0]->btncolor){ ?>
        ...
        #language-switcher input{
          color:<?php echo esc_attr($data[0]->btncolor) ?> !important;
          border-color:<?php echo esc_attr($data[0]->btncolor) ?> !important;
        }
        #language-switcher input:focus{
          box-shadow: none!important;}
        input[type="password"]:focus, input[type="text"]:focus, #rememberme:focus,.wp-core-ui .button.button-large:focus{
          outline: none!important;
          border: 1px solid <?php echo esc_attr($data[0]->btncolor) ?> !important;
          box-shadow: none!important;
        }
        ::selection{background:<?php echo esc_attr($data[0]->btncolor) ?>;}
        ...
        .login #backtoblog a:focus, .login #nav a:focus{
          border-bottom: 1px solid <?php echo esc_attr($data[0]->btncolor) ?> ;
          box-shadow: none !important;
        } <?php
      } ?>
